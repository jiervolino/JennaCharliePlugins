...
import jenna.client.BotBasicStrategy;
...
public class Huey implements IBot {
    ...
    protected BotBasicStrategy bs = new BotBasicStrategy();
    ...
    public void deal(Hid hid, Card card, int[] values) {
        ...
        // Track the Dealer's up-card to know when the game officially starts
        ...
        if (hid.getSeat() == mySeat) {
            ...
            // If upCard isn't null, it means the initial deal is over.
            // We only need to potentially make a play if we haven't busted or reached 21.
            if (this.upCard != null && myHand.getValue() < 21) {


                // If we have exactly 3 cards, we need to know if this card came from a Double Down.
                ...
                // If it wasn't a double down, it was a normal HIT, so we must make our next move
                play(hid);
            }
        }
    }
    ...
    public void play(final Hid hid) {
        ...
        // Spawn a worker thread so the single-threaded Dealer isn't blocked (Appendix 1, Rule 1)
